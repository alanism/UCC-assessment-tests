...
const App: React.FC = () => {
  ...
  const [ariaMessage, setAriaMessage] = useState<string>("Analyze the craft. Who is speaking? What is the mood?");
  const [timer, setTimer] = useState<number>(45); // Literature takes balanced time
  ...
  const handleSelection = (selectedIndex: number) => {
    ...
    if (isCorrect) {
      ...
      setAriaMessage("Exceptional analysis. You've decoded the author's structure.");
    } else {
      nextDifficulty = Math.max(1, difficulty - 1) as Difficulty;
      setAriaState(AriaState.Error);
      setAriaMessage("Perspective shift needed. Re-evaluate the tone and lens.");
    }
    ...
    const telemetry: ItemTelemetry = {
      ...
      distractorLogic: selectedIndex === -1 ? 'LITERAL_TRAP' : currentProblem.distractorLogicByOption[selectedIndex],
    ...
    setPillars(prev => ({
      ...
      speedBonuses: prev.speedBonuses + (isCorrect && timer > 20 ? 1 : 0),
      totalTime: prev.totalTime + (45 - timer),
      attempts: prev.attempts + 1,
      itemLog: [...prev.itemLog, telemetry],
      levelHistory: [...prev.levelHistory, { difficulty, time: 45 - timer, accuracy: isCorrect ? 1 : 0 }],
      crystalMemory: {
        ...prev.crystalMemory,
        mastered: isCorrect ? Array.from(new Set([...prev.crystalMemory.mastered, currentProblem.gradeMeta])) : prev.crystalMemory.mastered,
        operationsUsed: Array.from(new Set([...prev.crystalMemory.operationsUsed, currentProblem.lens])),
  ...
  useEffect(() => {
    if (sessionState !== 'review') {
      ...
      setAriaMessage("Discriminate the lens. How is the scene structured?");
      setTimer(45);
  ...
  const handleFinish = () => {
    ...
    setAriaMessage("Literature Evaluation complete. Review your craft and structural metrics.");
  };
  ...
  const mastery = (accuracy * 40) + ((difficulty / 5) * 40) + (speedScore * 20);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6 ucc-ui">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div>
          <h1 className="ucc-title text-3xl md:text-4xl lg:text-5xl text-[#111827] tracking-tighter uppercase">
            STAR Reading <span className="text-[#4EABBC]">×</span> Literature: Craft & Structure
          </h1>
          <p className="text-gray-500 text-xs mt-1 font-black uppercase tracking-[0.2em]">
            This instrument measures narrator, tone, figurative language, and structure.
          </p>
        </div>
              ...
              {sessionState === 'review' ? (
                ...
                <>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <h2 className="ucc-title text-xl text-[#111827]">Literary Passage</h2>
      ...
      <footer className="text-center text-gray-300 text-[10px] font-black uppercase tracking-[0.3em] py-12">
        Psychometric Literature Lab &copy; 2024
      </footer>
    </div>
  );
};
