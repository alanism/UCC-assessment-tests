...
const App: React.FC = () => {
  ...
  const [ariaMessage, setAriaMessage] = useState<string>("Analyze the word carefully. Context is key.");
  const [timer, setTimer] = useState<number>(30); // Vocab is faster than reading
  ...
  const handleSelection = (selectedIndex: number) => {
    ...
    if (isCorrect) {
      ...
      setAriaMessage("Excellent word choice. Your linguistic precision is high.");
    } else {
      nextDifficulty = Math.max(1, difficulty - 1) as Difficulty;
      setAriaState(AriaState.Error);
      setAriaMessage("Word confusion detected. Re-examine the roots and context.");
    }
    ...
    const telemetry: ItemTelemetry = {
      ...
      channel: currentProblem.channel,
      skillTag: currentProblem.skillTag,
      distractorType: currentProblem.distractorType,
      isCorrect,
      timeMs: timeTakenMs,
    ...
    setPillars(prev => ({
      ...
      totalTime: prev.totalTime + (30 - timer),
      attempts: prev.attempts + 1,
      itemLog: [...prev.itemLog, telemetry],
      levelHistory: [...prev.levelHistory, { difficulty, time: 30 - timer, accuracy: isCorrect ? 1 : 0 }],
      crystalMemory: {
        ...prev.crystalMemory,
        mastered: isCorrect ? Array.from(new Set([...prev.crystalMemory.mastered, currentProblem.grade])) : prev.crystalMemory.mastered,
        operationsUsed: Array.from(new Set([...prev.crystalMemory.operationsUsed, currentProblem.channel])),
  ...
  useEffect(() => {
    if (sessionState !== 'review') {
      ...
      setAriaMessage("Discriminate the meaning. Which choice is precise?");
      setTimer(30);
  ...
  const handleFinish = () => {
    ...
    setAriaMessage("Vocabulary Evaluation complete. Review your morphological and contextual metrics.");
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6 ucc-ui">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div>
          <h1 className="ucc-title text-3xl md:text-4xl lg:text-5xl text-[#111827] tracking-tighter uppercase">
            STAR Reading <span className="text-[#4EABBC]">×</span> Vocabulary
          </h1>
          <p className="text-gray-500 text-xs mt-1 font-black uppercase tracking-[0.2em]">
            This instrument measures word meaning, context discrimination, and morphology.
          </p>
        </div>
              ...
              {sessionState === 'review' ? (
                ...
                <>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <h2 className="ucc-title text-xl text-[#111827]">Vocabulary Item</h2>
                      <SwissClockTimer secondsRemaining={timer} totalSeconds={30} />
                    </div>
                    
                    <p className="font-bold text-lg text-[#111827] pt-2 min-h-[4rem]">
                      {currentProblem.stem}
                    </p>
      ...
      <footer className="text-center text-gray-300 text-[10px] font-black uppercase tracking-[0.3em] py-12">
        Psychometric Vocabulary Lab &copy; 2024
      </footer>
    </div>
  );
};
