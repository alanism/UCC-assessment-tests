...
const App: React.FC = () => {
  ...
  const [ariaMessage, setAriaMessage] = useState<string>("Read the text carefully. Focus on structure.");
  const [timer, setTimer] = useState<number>(60); // Reading takes longer
  ...
  const handleSelection = (selectedIndex: number) => {
    ...
    if (isCorrect) {
      ...
      setAriaMessage("Accurate analysis. Your brain read the architecture correctly.");
    } else {
      nextDifficulty = Math.max(1, difficulty - 1) as Difficulty;
      setAriaState(AriaState.Error);
      setAriaMessage("Logic gap. Consider the signal words used in the text.");
    }

    const telemetry: ItemTelemetry = {
      id: currentProblem.id,
      grade: currentProblem.grade,
      structure: currentProblem.structure,
      skill: currentProblem.skill,
      authorPurpose: currentProblem.authorPurpose,
  ...
  useEffect(() => {
    if (sessionState !== 'review') {
      ...
      setAriaMessage("Analyze the craft. What is the author's blueprint?");
      setTimer(60);
  ...
  const handleFinish = () => {
    ...
    setAriaMessage("Evaluation complete. Review your structural reading metrics.");
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6 ucc-ui">
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div>
          <h1 className="ucc-title text-3xl md:text-4xl lg:text-5xl text-[#111827] tracking-tighter uppercase">
            STAR Reading <span className="text-[#4EABBC]">×</span> Informational: Craft & Structure
          </h1>
          <p className="text-gray-500 text-xs mt-1 font-black uppercase tracking-[0.2em]">
            This instrument measures text architecture and structural reasoning.
          </p>
        </div>
      ...
      <footer className="text-center text-gray-300 text-[10px] font-black uppercase tracking-[0.3em] py-12">
        Psychometric Informational Text Lab &copy; 2024
      </footer>
    </div>
  );
};
