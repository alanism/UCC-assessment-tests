...
const App: React.FC = () => {
  ...
  const [ariaMessage, setAriaMessage] = useState<string>("Analyze the key ideas. Look for supporting details.");
  ...
  const handleSelection = (selectedIndex: number) => {
    ...
    if (isCorrect) {
      ...
      setAriaMessage("Accurate extraction. You've identified the core meaning.");
    } else {
      nextDifficulty = Math.max(1, difficulty - 1) as Difficulty;
      setAriaState(AriaState.Error);
      setAriaMessage("Missing context. Re-examine the passage for specific details.");
    }
  ...
  useEffect(() => {
    if (sessionState !== 'review') {
      ...
      setAriaMessage(`Identify the ${currentProblem.lens.replace('_', ' ').toLowerCase()}.`);
      setTimer(45);
  ...
  const handleFinish = () => {
    ...
    setAriaMessage("Assessment complete. Review your Key Ideas & Details performance.");
  };
  ...
  const mastery = (accuracy * 40) + ((difficulty / 5) * 30) + (inferenceAccuracy * 30);
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-1 font-black uppercase tracking-[0.2em]">
            This instrument measures main ideas, key details, and inferential logic.
          </p>
        </div>
